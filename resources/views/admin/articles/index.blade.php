    ...
    <div class="page-header mt-3">
        <div class="page-header-content header-elements-inline">
            <div class="page-title py-1 d-flex">
                <h5><span>@lang('app.Articles') </span> </h5>
            </div>

            <div class="header-elements d-none">
                <div class="d-flex justify-content-center">
                    <a href="{{ asset('admin/articles/new') }}" class="btn bg-teal-400 btn-labeled btn-labeled-left rounded-round"><b><i class="icon-plus3"></i></b> @lang('app.Add_Article') </a>
                </div>
            </div>
        </div>
                ...
                <div class="card">
                    <table class="table table-isp table-hover table-bordered table-xs">
                        ...
                        <tr>
                            ...
                            <th class="text-right">@lang('app.Created_at')</th>
                        ...
                        <tbody>
                            @foreach($items as $each)
                                <tr>
                                    ...
                                    <td>{{ $each->name }}</td>
                                    <td>{{ $each->name_en }}</td>
                                    <td>@if(isset($each->category)) {{ $each->category->name }} @endif</td>
                                    <td class="text-center"> {{ $each->views }} </td>
                                    <td class="text-center">
                                        @if($each->status == 1)
                                            <span class="badge badge-success">@lang('app.Active')</span>
                                        @else
                                            <span class="badge badge-danger">@lang('app.Inactive')</span>
                                        @endif
                                    </td>
                                    <td class="text-right"> {{ date('d M Y H:i', strtotime($each->created_at)) }} </td>
                                    <td class="text-center table-icons">
                                        <ul class="list-group list-group-horizontal icons-list">
                                            <li class="list-group-item"><a class="text-primary" target="_blank" href="{{ asset('article/'. $each->id ) }}"><i class="icon-eye4"></i></a></li>
                                            <li class="list-group-item"><a class="text-primary" href="{{ asset('admin/articles/update/'. $each->id ) }}"><i class="icon-pencil7"></i></a></li>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form method="post" action="{{ asset('admin/articles/delete') }}" id="delete-form" >
                        @csrf
