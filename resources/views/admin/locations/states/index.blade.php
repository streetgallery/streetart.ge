    ...
    <div class="page-header mt-3">
        <div class="page-header-content header-elements-inline">
            <div class="page-title py-1 d-flex">
                <h5><span>@lang('app.States') </span> </h5>
            </div>

            <div class="header-elements d-none">
                <div class="d-flex justify-content-center">
                    <a href="{{ asset('admin/states/add') }}" class="btn bg-teal-400 btn-labeled btn-labeled-left rounded-round"><b><i class="icon-plus3"></i></b> @lang('app.Add_State') </a>
                </div>
            </div>
        </div>
                        ...
                        <tbody>
                            @foreach($items as $each)
                                <tr>
                                    ...
                                    <td><a href="{{ asset('admin/cities?state_id='. $each->id) }}">{{ $each->name }}</a></td>
                                    <td><a href="{{ asset('admin/cities?state_id='. $each->id) }}">{{ $each->name_en }}</a></td>
                                    <td>@if(isset($each->country)) <a href="{{ asset('admin/states?country_id='. $each->country->id) }}">{{ $each->country->name }}</a>@endif</td>
                                    ...
                                    <td class="text-center table-icons">
                                        <ul class="list-group list-group-horizontal icons-list">
                                            <li class="list-group-item"><a class="text-primary" href="{{ asset('admin/states/update/'. $each->id ) }}"><i class="icon-pencil7"></i></a></li>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form method="post" action="{{ asset('admin/states/delete') }}" id="delete-form" >
                        @csrf
