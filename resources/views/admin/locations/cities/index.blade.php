    ...
    <div class="page-header mt-3">
        <div class="page-header-content header-elements-inline">
            <div class="page-title py-1 d-flex">
                <h5><span>@lang('app.Cities') </span> </h5>
            </div>

            <div class="header-elements d-none">
                <div class="d-flex justify-content-center">
                    <a href="{{ asset('admin/cities/add') }}" class="btn bg-teal-400 btn-labeled btn-labeled-left rounded-round"><b><i class="icon-plus3"></i></b> @lang('app.Add_City') </a>
                </div>
            </div>
        </div>
                ...
                <div class="card">
                    <table class="table table-isp table-hover table-bordered table-xs">
                        ...
                        <tr>
                            ...
                            <th>@lang('app.City')</th>
                            <th>@lang('app.City') @lang('app.Eng') </th>
                        ...
                        @foreach($items as $each)
                            <tr>
                                ...
                                <td>{{ $each->name }}</td>
                                <td>{{ $each->name_en }}</td>
                                <td>@if(isset($each->state))  <a href="{{ asset("admin/cities?state_id=" . $each->state->id ) }}">{{ $each->state->name }}</a> @endif</td>
                                <td>@if(isset($each->state->country)) <a href="{{ asset("admin/cities?country_id=" . $each->state->country->id ) }}">{{ $each->state->country->name }}</a>  @endif</td>
                                ...
                                <td class="text-center table-icons">
                                    <ul class="list-group list-group-horizontal icons-list">
                                        <li class="list-group-item"><a class="text-primary" href="{{ asset('admin/cities/update/'. $each->id ) }}"><i class="icon-pencil7"></i></a></li>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form method="post" action="{{ asset('admin/cities/delete') }}" id="delete-form" >
                        @csrf
