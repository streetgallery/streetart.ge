...
class ArticleContentImageRepository
{
    public function upload($form_data)
    {
        $validator = Validator::make($form_data, Image::$rulesProductContent, Image::$messagesProductContent);
        if ($validator->fails()) {
        ...
        $sessionImage->type = "articleContent";
        $sessionImage->original = "/files/articles/". $form_data['item_id'] . "/content/" . $allowed_filename;
        $sessionImage->save();

        $url = "/files/articles/". $form_data['item_id'] ."/content/".$allowed_filename ;
    ...
    {
        $full_size_dir = public_path('files/articles/'. $item_id .'/content/');
    ...
    public function original($photo, $filename, $item_id)
    {
        ...
        $image = $manager->make($photo)->save(public_path('files/articles/'. $item_id .'/content/') . $filename);
        return $image;
    }

    public function delete($filename,$item_id)
    {
        $original_dir = public_path('files/articles/'. $item_id .'/content/');
