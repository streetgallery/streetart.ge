...
class AvatarImageRepository
{
    public function upload($form_data)
    {
        ...
        if ($validator->fails()) {
            ...
            if(\App::isLocale('ka')){
                return back()->with('alert_fail', $validator->messages()->first());
            } else {
                return back()->with('alert_fail', $validator->messages()->first());
            }
        ...
        if (!$uploadSuccess1 || !$uploadSuccess2 || !$uploadSuccess3) {
            ...
            /// ძველის წაშლა
        $original_dir = public_path('files/users/'. Auth::user()->id .'/original/');
        $medium_dir = public_path('files/users/'. Auth::user()->id .'/medium/') ;
        $small_dir = public_path('files/users/'. Auth::user()->id .'/small/') ;

        $item = User::find(Auth::user()->id);
        foreach ($item->images as $image) {
            $full_path1 = $original_dir . $image->filename;
            $full_path2 = $medium_dir . $image->filename;
            $full_path3 = $small_dir . $image->filename;
        ...
        if(\App::isLocale('ka')){
            return back()->with('alert_ok', 'ავატარი წამატებით განახლდა');
        } else {
            return back()->with('alert_ok', 'Avatar updated successfully');
        }
