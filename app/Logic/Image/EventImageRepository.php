...
class EventImageRepository
{
    public function upload($form_data)
    {
        ...
        $sessionImage->original = "/files/events/". $form_data['item_id'] . "/original/" . $allowed_filename;
        $sessionImage->medium   = "/files/events/". $form_data['item_id'] . "/medium/" . $allowed_filename;
        $sessionImage->small    = "/files/events/". $form_data['item_id'] . "/small/" . $allowed_filename;
        $sessionImage->save();
        return Response::json([
            'error' => false,
            'code' => 200,
    ...
    {
        $full_size_dir = public_path('files/events/'. $item_id .'/original/');
    ...
    public function original($photo, $filename, $item_id)
    {
        ...
        $image = $manager->make($photo)->save(public_path('files/events/'. $item_id .'/original/') . $filename);
        return $image;
    }
    ...
    public function medium($photo, $filename, $item_id)
    {
        ...
        })->save(public_path('files/events/'. $item_id .'/medium/') . $filename);
        return $image;
    }
    ...
    public function small($photo, $filename, $item_id)
    {
        ...
        $image = $manager->make($photo)->fit(200, 200)
            ->save(public_path('files/events/'. $item_id .'/small/') . $filename);
        return $image;
    }

    public function delete($filename,$item_id)
    {
        $original_dir = public_path('files/events/'. $item_id .'/original/');
        $medium_dir = public_path('files/events/'. $item_id .'/medium/') ;
        $small_dir = public_path('files/events/'. $item_id .'/small/') ;
