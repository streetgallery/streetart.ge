...
class ArticleImageRepository
{
    public function upload($form_data)
    {
        ...
        $path = public_path('files/articles/' .$form_data['item_id'] );
        ...
        $sessionImage->original = "/files/articles/". $form_data['item_id'] . "/original/" . $allowed_filename;
        $sessionImage->medium   = "/files/articles/". $form_data['item_id'] . "/medium/" . $allowed_filename;
        $sessionImage->small    = "/files/articles/". $form_data['item_id'] . "/small/" . $allowed_filename;
        $sessionImage->save();
        return Response::json([
            'error' => false,
            'code' => 200,
    ...
    {
        $full_size_dir = public_path('files/articles/'. $item_id .'/original/');
    ...
    public function original($photo, $filename, $item_id)
    {
        ...
        $image = $manager->make($photo)->save(public_path('files/articles/'. $item_id .'/original/') . $filename);
        return $image;
    }
    ...
    {
        $manager = new ImageManager();
        $image = $manager->make($photo)->resize(990, null, function ($constraint) {
            $constraint->aspectRatio();
        })->save(public_path('files/articles/'. $item_id .'/medium/') . $filename);
        return $image;
    }
    ...
    {
        $manager = new ImageManager();
        $image = $manager->make($photo)->fit(350, 300)
            ->save(public_path('files/articles/'. $item_id .'/small/') . $filename);
        return $image;
    }

    public function delete($filename,$item_id)
    {
        $original_dir = public_path('files/articles/'. $item_id .'/original/');
        $medium_dir = public_path('files/articles/'. $item_id .'/medium/') ;
        $small_dir = public_path('files/articles/'. $item_id .'/small/') ;
